...
const verifyOptions = async () => {
  ...
  if (!metadata && !env) {
    ...
    if (
      savedAWSProfiles.length &&
      !savedAWSProfiles.includes(profile) &&
      (!key || !secret)
    ) {
  ...
  if (!userpool) {
    ...
    if (profile) {
      ...
      throw Error(
        "No credential found. Try using --help to read about various configurations supported."
      );
    }
